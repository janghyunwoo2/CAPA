...
인터페이스: Iterator → {"timestamp": datetime, "click_count": int}
"""
import time
import logging
import random
...
# 베이스라인: 5분당 기본 click 수 (실제 로그 제네레이터 스케일에 맞춤)
BASE_CLICKS = 250
...
def generate_mock_data(history_days: int = 14, seed: int = 42) -> list[dict]:
    ...
    Returns:
        list of {"timestamp": datetime, "click_count": int}
    ...
    for i in range(num_windows):
        ...
        base_count = int(BASE_CLICKS * multiplier * noise)
...
class MockKinesisSource:
    ...
    - 반환 형식: {"timestamp": datetime, "click_count": int}
    """
