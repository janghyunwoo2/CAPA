...
class ProphetDetector:
    ...
    def train(self, records: list[dict]) -> None:
        ...
        Args:
            records: [{"timestamp": datetime, "impression_count": int}, ...]
        ...
        try:
            df = pd.DataFrame([
                {"ds": r["timestamp"], "y": r["impression_count"]}
                for r in records
            ])
    ...
    def predict(self, timestamp: datetime, value: float) -> dict:
        ...
        with self._lock:  # thread safety
            ...
            try:
                ...
                # 상단 신뢰구간에 가중치(예: 1.2배)를 적용하여 Spike 감지 기준을 완화
...
class IsolationForestDetector:
    ...
    def train(self, records: list[dict]) -> None:
        ...
        try:
            values = np.array([r["impression_count"] for r in records]).reshape(-1, 1)
