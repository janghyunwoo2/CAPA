...
class ProphetDetector:
    ...
    def train(self, records: list[dict]) -> None:
        ...
        Args:
            records: [{"timestamp": datetime, "conversion_count": int}, ...]
        ...
        try:
            df = pd.DataFrame([
                {"ds": r["timestamp"], "y": r["conversion_count"]}
                for r in records
            ])
...
class IsolationForestDetector:
    ...
    def train(self, records: list[dict]) -> None:
        ...
        try:
            values = np.array([r["conversion_count"] for r in records]).reshape(-1, 1)
