...
인터페이스: Iterator → {"timestamp": datetime, "conversion_count": int}
"""
import time
import logging
import random
...
# 베이스라인: 5분당 기본 전환 수 (전환은 노출보다 적으므로 낮게 설정)
BASE_CONVERSIONS = 5
...
def get_traffic_multiplier(hour: int, weekday: int) -> float:
    ...
    Returns:
        트래픽 배율 (1.0 기준)
    """
    # 시간대별 기본 배율 (스케일다운: 평균 5건, 피크 10건)
    if 11 <= hour < 13:
        base = 2.0      # 점심 피크: 약 10건
    elif 18 <= hour < 22:
        base = 2.0      # 저녁 피크: 약 10건
    elif 22 <= hour < 24:
        base = 1.2      # 야식 시간: 약 6건
    elif 2 <= hour < 6:
        base = 0.5      # 새벽: 약 2~3건
...
def generate_mock_data(history_days: int = 14, seed: int = 42) -> list[dict]:
    ...
    Returns:
        list of {"timestamp": datetime, "conversion_count": int}
    ...
    for i in range(num_windows):
        ...
        base_count = int(BASE_CONVERSIONS * multiplier * noise)
...
class MockKinesisSource:
    ...
    - 반환 형식: {"timestamp": datetime, "conversion_count": int}
    """
