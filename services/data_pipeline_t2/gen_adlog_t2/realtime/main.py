...
def get_traffic_multiplier(timestamp: datetime) -> float:
    ...
    if 0 <= hour < 1:
        ...
        hour_mult = random.uniform(0.2, 0.3)  # 새벽  <- 0.1-0.2
    elif 6 <= hour < 7:
        ...
        hour_mult = random.uniform(0.7, 1.6)  # 점심 완충지대
    elif 14 <= hour < 17:
        hour_mult = random.uniform(0.8, 0.9)  # 오후 <- 0.6-0.8
    elif 17 <= hour < 18:
        hour_mult = random.uniform(0.8, 2.0)  # 오후 완충지대
    elif 18 <= hour < 21:
