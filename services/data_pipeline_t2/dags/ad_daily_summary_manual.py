    ...
    create_daily_summary = KubernetesPodOperator(
        ...
        env_vars={
            ...
            "QUERIES": (
                ...
                "conversion_agg AS ( "
                "  SELECT campaign_id, device_type, COUNT(DISTINCT event_id) AS conversions "
                ...
                "  GROUP BY campaign_id, device_type ) "
                "SELECT h.campaign_id, h.device_type, '{{ params.target_date }}' AS ds, "
                "       h.impressions, h.clicks, COALESCE(c.conversions, 0) AS conversions, "
                "       CASE WHEN h.impressions > 0 THEN CAST(h.clicks AS DOUBLE) / CAST(h.impressions AS DOUBLE) * 100 ELSE 0.0 END AS ctr, "
                "       CASE WHEN h.clicks > 0 THEN CAST(COALESCE(c.conversions, 0) AS DOUBLE) / CAST(h.clicks AS DOUBLE) * 100 ELSE 0.0 END AS cvr "
                "FROM hourly_agg h LEFT JOIN conversion_agg c ON h.campaign_id = c.campaign_id AND h.device_type = c.device_type"
                "|||"
                "DROP TABLE IF EXISTS {{ params.database }}.ad_daily_summary_tmp"
            ),
        },
