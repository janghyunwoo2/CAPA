"""
DAG: t2_ad_hourly_summary (KubernetesPodOperator 버전)

EKS Airflow 배포용. 매시간 광고 로그를 집계(HourlyETL)합니다.
기존 PythonOperator(01_ad_hourly_summary.py)와 동일한 비즈니스 로직을
...
# ── 3. DAG 정의 ──────────────────────────────────────────────────────────────
with DAG(
    dag_id="t2_ad_hourly_summary_v2",
    default_args=default_args,
    description="[EKS] T2 HourlyETL — 매시간광고 로그 집계 (KubernetesPodOperator)",
    schedule="10 * * * *",  # 매시간 10분 (UTC)
    start_date=pendulum.datetime(2026, 2, 13, tz="UTC"),
    catchup=False,
    max_active_runs=1,
    tags=["capa", "t2", "hourly", "etl", "eks"],
) as dag:
    run_hourly_etl = KubernetesPodOperator(
