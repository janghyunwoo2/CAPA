...
def send_report_to_slack(pdf_path: str, report_date: str = None, report_type: str = "daily", only_upload: bool = False) -> bool:
    ...
    try:
        ...
        if only_upload:
            ...
            dashboard_url = os.getenv("DAILY_DASHBOARD_URL", "")
            sub_url = os.getenv("PERIOD_DASHBOARD_URL", "")
            title = f"CAPA 광고 성과 보고서 - [{label}] ({report_date})"
            comment = f"{emoji} [{label}] {report_date} 보고서 생성 완료"
            if dashboard_url:
                comment += f"\n🔗 *일간 대시보드*: <{dashboard_url}|클릭하여이동>"
            if sub_url:
                comment += f"\n🔗 *기간 대시보드*: <{sub_url}|클릭하여이동>"
        else:
            dashboard_url = os.getenv("DAILY_DASHBOARD_URL", "")
            sub_url = os.getenv("PERIOD_DASHBOARD_URL", "")
...
def send_combined_notification(report_types: list[str], report_date: str) -> bool:
    ...
    try:
        ...
        message = (
            ...
            f"🔗 *일간 대시보드*: <{dashboard_url}|클릭하여이동>\n"
        )
        sub_url = os.getenv("PERIOD_DASHBOARD_URL", "")
        if sub_url:
            message += f"🔗 *기간 대시보드*: <{sub_url}|클릭하여이동>\n"
