...
dag = DAG(
    dag_id='t3_daily_report',
    default_args=default_args,
    description='T3 일간 보고서 - 매일 아침 08:00 KST',
    schedule_interval=None,  # 비활성화 → t3_report_generator_v3로 대체
    catchup=False,
    tags=['report-generator', 't3', 'daily'],
)
...
def run(**context):
    ...
    if result['status'] != 'success':
        ...
        raise AirflowException(f"일간 보고서 생성 실패: {result.get('error')}")
