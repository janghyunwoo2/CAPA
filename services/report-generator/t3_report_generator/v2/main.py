...
def generate_report(date: datetime = None) -> str:
    ...
    logger.info(f"보고서 생성 시작 (입력 날짜): {date.strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"현재 시스템 시간 (datetime.now): {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main(date_str: str = None) -> dict[str, Any]:
    ...
    try:
        ...
        # 보고서 생성 (일반적인 generate_report는 markdown만 반환)
        markdown = generate_report(date)

        # =========================================================================
        # 5단계: 일별 데이터 추출 (PDF 차트용)
        # =========================================================================
        # 데이터 범위 결정
        is_first_day = is_month_start(date)
        month_start = get_month_start(date)
        ...
        try:
            ...
            logger.warning(f"차트용 일별 데이터 조회 실패: {e}")
            daily_breakdown = []
...
if __name__ == "__main__":
    ...
    if result["status"] == "success":
        ...
        try:
            ...
            print("[보고서 출력 중 인코딩 에러 발생 - 파일로는 정상 저장됨]")

        # 1. 마크다운 파일 저장 (예: report_2026-03-02.md)
        md_filename = f"report_{result['date']}.md"
        try:
            with open(md_filename, "w", encoding="utf-8") as f:
                f.write(result["markdown"])
            print(f"\n[안내] 마크다운 보고서 저장: {md_filename}")
        except Exception as e:
            print(f"\n[주의] 마크다운 파일 저장 중 오류 발생: {e}", file=sys.stderr)

        # 2. PDF 파일 생성 (예: report_2026-03-02.pdf)
        pdf_filename = f"report_{result['date']}.pdf"
        try:
            # daily_breakdown 추출 (없으면 None)
            ...
            print(f"[안내] PDF 보고서 저장: {pdf_path}")

            # 3. Slack으로 전송
            if slack_notifier.send_report_to_slack(pdf_path, result["date"]):
                print(f"[안내] Slack 전송 완료")
            else:
                print(f"[안내] Slack 전송 실패 또는 설정 안 됨")
        except Exception as e:
            print(f"\n[주의] PDF 생성 중 오류 발생: {e}", file=sys.stderr)
    else:
