...
def get_daily_kpi(start_date: str, end_date: str) -> dict[str, Any]:
    ...
    """
    # 1. 누적 KPI 조회 (전월 데이터를 비교 기간으로 사용)
    ...
    return {
        ...
        "prev_summary": df_prev.to_dict(orient="records")[0] if len(df_prev) > 0 else {},
...
def get_weekly_list(month_start: str, end_date: str) -> list[dict[str, Any]]:
    ...
    for i, week in enumerate(weeks):
        ...
        # 전주 데이터 조회 (이전 주차가 있으면)
        prev_week_kpi = None
        if i > 0:
            prev_week = weeks[i - 1]
            prev_week_kpi = get_daily_kpi(prev_week["start_date"], prev_week["end_date"])
