...
# 로컬 모듈 임포트
from data_pipeline_t2.dags.script.generate_sample_logs import AdLogGenerator
from data_pipeline_t2.dags.script.processor import AdDataProcessor
from analyzer import AdAnalytics
from data_pipeline_t2.dags.script.visualize import plot_ctr_over_time
...
def task_generate_logs(**context) -> None:
    ...
    Returns:
        None (부작용: data/raw/logs.parquet 생성)
    """
    logger.info("[Task 1] 샘플 로그 생성 시작...")
    gen = AdLogGenerator(out_path="data/raw/logs.parquet")
    gen.generate(n=20000)  # 20K 샘플 이벤트
    logger.info("[Task 1] 샘플 로그 생성 완료: data/raw/logs.parquet")
...
def task_process_metrics(**context) -> None:
    ...
    processor = AdDataProcessor(
        raw_path="data/raw/logs.parquet",
        out_path="data/processed/metrics.parquet"
    )
    processor.process()
    logger.info("[Task 2] 메트릭 처리 완료: data/processed/metrics.parquet")
...
def task_generate_report(**context) -> None:
    ...
    analyzer = AdAnalytics(
        metrics_path="data/processed/metrics.parquet",
        out_csv="data/analysis/report_top_ads.csv"
    )
    analyzer.generate_report(top_n=20)
    logger.info("[Task 3] 분석 리포트 생성 완료: data/analysis/report_top_ads.csv")
...
def task_visualize_outputs(**context) -> None:
    ...
    logger.info("[Task 4] 시각화 생성 완료: data/outputs/daily_ctr.png, top5_ads_ctr.png")
...
with DAG(
    ...
    schedule_interval=None,  # 수동 실행 또는 스케줄 설정 가능 (예: "0 2 * * *" = 매일 2시)
