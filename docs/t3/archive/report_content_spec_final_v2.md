# Report Generator - 리포트 콘텐츠 상세 스펙 (최종)

**작성일**: 2026-03-09
**작성자**: Developer
**상태**: 최종 승인 (Reviewer 검수 완료 - 95점)
**변경 이력**: v1 -> v2 (Reviewer 피드백 7건 + 추가 2건 반영) -> 최종 승인
...
성과와 경고를 균형 있게 2+2로 표시:

```
[성과] 분식 카테고리 매출 전주 대비 +15.2% 성장 (전 카테고리 중 최고 성장)
[성과] 전체 ROAS 342.1% -> 목표 300% 초과 달성

[경고] 카페 카테고리 CTR 전주 대비 -12.3% 하락 (5.68% -> 5.0%) -> 광고 소재 점검 필요
[경고] shop_0891 ROAS 45%로 손익분기점(100%) 이하 -> 캠페인 일시중지 검토
```

**자동 생성 규칙 (LLM 사용 안 함, if-else 규칙):**
- [성과] 선정 기준:
  - 카테고리 중 매출 변화율 최고인 항목
  - 목표 달성율 최고인 KPI
- [경고] 선정 기준:
  - 전기 대비 CTR 또는 CVR 10% 이상 하락한 카테고리
  - ROAS 100% 이하인 상점 중 노출 100건 이상인 최하위 상점
...
**이상 표시 규칙 (PDF 셀 색상):**
- 전기 대비 10% 이상 하락한 지표: 빨간색 배경 (#FFE0E0)
- 전기 대비 20% 이상 상승한 지표: 초록색 배경 (#E0FFE0)
...
### 5-2. 이상 감지 표시 규칙 (PDF 셀 색상)

| 조건 | 표시 |
|---|---|
| 전기 대비 CTR 또는 CVR 10% 이상 하락 | 해당 셀 빨간색 배경 (#FFE0E0) + 볼드 |
| 전기 대비 매출 15% 이상 상승 | 해당 셀 초록색 배경 (#E0FFE0) + 볼드 |
| ROAS 200% 이하 | 해당 셀 주황색 배경 (#FFF3E0) |

위 예시에서:
- 분식 매출 +15.2%: 초록색 강조
- 카페 CTR 5.0%: 빨간색 표시 (전주 대비 하락)

### 5-3. 카테고리별 매출 비교 바 차트
...
**근거:** TIER 2 - "카테고리별 성과 테이블 (순위, 수치)" + TIER 3 "캠페인별 성과"
...
### 6-3. 이상 감지 표시 규칙 (PDF 셀 색상)

| 조건 | 표시 |
|---|---|
| ROAS < 100% (손익분기점 이하) | "손실" 빨간색 태그 (#FFE0E0) |
| ROAS 100~200% | "주의" 주황색 태그 (#FFF3E0) |
| ROAS > 400% | "우수" 초록색 태그 (#E0FFE0) |

**Bottom 10 추가 정보:**
- 전주 대비 ROAS 추이 (개선 중인지 악화 중인지)
- 노출 대비 전환율이 극단적으로 낮은 경우 별도 표시
...
## 11. 액션 아이템 (주간: 섹션 8 / 월간: 섹션 11)

**목적**: 팀장이 "그래서 뭘 해야 하나?"에 대한 구체적 가이드 제공

**방식**: 규칙 기반 자동 생성. Claude LLM 사용 안 함 (지표 중심 방침 준수).
...
| 조건 | 액션 아이템 | 우선순위 |
|---|---|---|
| ROAS < 100% 상점 (노출 100건 이상) | "{shop_id} 캠페인 일시중지 검토 (ROAS {값}%)" | 긴급 |
| ROAS 100~200% 상점 (노출 100건 이상) | "{shop_id} 입찰가 하향 조정 검토 (ROAS {값}%)" | 높음 |
| CPC 전기 대비 15% 이상 상승 | "CPC 상승 추이 모니터링 ({전기}원 -> {현재}원)" | 중간 |

**효율 관련 액션:**

| 조건 | 액션 아이템 | 우선순위 |
|---|---|---|
| 카테고리 CTR 전기 대비 10% 이상 하락 | "{카테고리} 광고 소재 점검 필요 (CTR {전기}% -> {현재}%)" | 높음 |
| 카테고리 CVR 전기 대비 10% 이상 하락 | "{카테고리} 랜딩 페이지/상품 구성 점검 (CVR {전기}% -> {현재}%)" | 중간 |
| 퍼널 특정 단계 전환율 전기 대비 5%p 이상 하락 | "{단계} 전환율 하락 원인 분석 필요" | 중간 |

**성과 관련 액션:**

| 조건 | 액션 아이템 | 우선순위 |
|---|---|---|
| ROAS > 500% 상점 | "{shop_id} 예산 증액 검토 (ROAS {값}%)" | 중간 |
| 카테고리 매출 전기 대비 20% 이상 성장 | "{카테고리} 성장세 유지 위해 예산 확대 검토" | 참고 |
| 목표 달성율 < 80% | "{지표} 목표 미달 ({달성율}%) - 긴급 개선 필요" | 긴급 |

### 11-2. 출력 예시

| 우선순위 | 액션 | 대상 | 근거 |
|---|---|---|---|
| 긴급 | 캠페인 일시중지 검토 | shop_0891 (카페) | ROAS 45% (기준: 100% 미만) |
| 높음 | 광고 소재 점검 | 카페 카테고리 | CTR 5.68% -> 5.0% (기준: 10% 이상 하락) |
| 높음 | 입찰가 하향 조정 검토 | shop_0732 (중식) | ROAS 162% (기준: 200% 미만) |
| 중간 | CPC 상승 추이 모니터링 | 전체 | 46.9원 -> 50.8원, +8.3% |
| 중간 | 예산 증액 검토 | shop_0001 (치킨) | ROAS 520% (기준: 500% 이상) |
| 참고 | 예산 확대 검토 | 분식 카테고리 | 매출 +15.2% (기준: 20% 이상 성장) |

### 11-3. config.yaml Threshold 설정

```yaml
alert_rules:
  roas_stop_threshold: 100      # ROAS(%) 이하 시 일시중지 권고
  roas_reduce_threshold: 200    # ROAS(%) 이하 시 입찰가 하향 권고
  roas_increase_threshold: 500  # ROAS(%) 이상 시 예산 증액 권고
  ctr_drop_alert: 10            # CTR 하락률(%) 이상 시 경고
  cvr_drop_alert: 10            # CVR 하락률(%) 이상 시 경고
  cpc_increase_alert: 15        # CPC 상승률(%) 이상 시 경고
  funnel_drop_alert: 5          # 퍼널 전환율 하락(%p) 이상 시 경고
  target_critical: 80           # 목표 달성율(%) 미만 시 긴급
  min_impressions: 100          # 상점 분석 최소 노출수
```

### 11-4. 구현 방식

- Python 코드에서 if-else 규칙으로 구현 (`alert_engine.py` 신규 파일)
- 각 규칙의 threshold는 `config.yaml`에서 로드
- 우선순위: 긴급 > 높음 > 중간 > 참고
- 최대 10개 액션 아이템 출력 (우선순위순 정렬)
...
| 파일 | 용도 |
|---|---|
| `config.yaml` | 목표치 + 알림 threshold 설정 |
| `alert_engine.py` | 규칙 기반 액션 아이템 자동 생성 모듈 |
...
| `pdf_exporter.py` | 차트 2개 추가 (효율 차트, 시간대별), 이상 감지 셀 색상, 액션 아이템 섹션 |
...
| 리포트 유형 | 쿼리 수 | 상세 |
|---|---|---|
| 주간 | 8개 | KPI(현재+전주) + 일별 + 카테고리(현재+전주) + 상점(현재+전주) + 퍼널 |
| 월간 | 12개 | 위 8개 + 디바이스 + 시간대별 + 비용(현재+전월) |
