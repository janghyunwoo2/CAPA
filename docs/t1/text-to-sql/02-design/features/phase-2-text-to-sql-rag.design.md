...
### 1.4 Phase 1 → Phase 2 전환 플래그

Phase 2의 모든 주요 기능은 환경변수 플래그로 개별 활성화할 수 있다. 전환 기간 동안 Phase 1 동작을 유지하며 점진적으로 활성화한다.

| 플래그 | 기본값 | 활성화 시 동작 |
|--------|--------|--------------|
| `PHASE2_RAG_ENABLED` | `false` | Step 4에서 3단계 RAG(`retrieve_v2()`) 사용 |
| `ASYNC_QUERY_ENABLED` | `false` | POST /query가 202 비동기로 전환 |
| `DYNAMODB_ENABLED` | `false` | History/Feedback 저장소를 DynamoDB로 전환 |

> **전환 순서 권장**: `DYNAMODB_ENABLED=true` (데이터 계층 먼저) → `PHASE2_RAG_ENABLED=true` (검색 품질) → `ASYNC_QUERY_ENABLED=true` (비동기 전환)
...
| FR-13 | 비즈니스 용어 지속 관리 | Admin API + Airflow Task 4 |
| FR-14 | Athena 특화 지식 지속 관리 | Admin API + Airflow Task 4 |
| FR-15 | 정책 데이터 지속 관리 | Admin API + Airflow Task 4 |
| FR-16 | 피드백 루프 품질 제어 | 즉시 학습 폐기 → Airflow 배치 검증 |
| FR-17 | 중복 쿼리 방지 | SQL 해시 기반 Redash 쿼리 재사용 |
| FR-18 | Airflow DAG 연동 | `capa_chromadb_refresh` DAG 신규 |
| - | BackgroundTasks 비동기 응답 | POST /query → 202 + 폴링 |
| - | History 저장소 전환 | JSON Lines → DynamoDB |
...
| Step 7 (Redash 쿼리 생성) | 매번 신규 생성 | **SQL 해시 기반** 기존 쿼리 재사용 (FR-17) |
...
| [신규] Admin API | 없음 | ChromaDB 지속 관리용 (FR-13~15) |
...
│  │              │          │  POST /feedback                          │      │
│  │              │          │  POST /admin/training-data/{type} [신규] │      │
│  └──────────────┘          └──────────────┬───────────────────────────┘      │
└───────────────────────────────────────────┼──────────────────────────────────┘
                                            │
┌───────────────────────────────────────────┼──────────────────────────────────┐
...
│  │  - Few-shot  │  │  - S3 결과   │  │  - 실행/폴링 │  │  - QueryHash │      │
│  │  - 용어 사전 │  │              │  │  - 시각화    │  │  - AsyncTask │      │
│  │  - 정책 문서 │  │              │  │              │  │              │      │
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘      │
│                                                                                │
...
## 4. FR-13~15: ChromaDB 지속 관리 설계

> **Phase 1 대비 변경점**: `seed_chromadb.py` 1회 적재 → Admin API + Airflow Task 4 주기 갱신 체계로 전환.

### 4.1 관리 대상 및 방식

| FR | 관리 대상 | Phase 1 | Phase 2 추가 체계 |
|----|---------|---------|-----------------|
| FR-13 | 비즈니스 용어 (CTR, ROAS 등) | `seed_chromadb.py` 1회 적재 | Admin API + Airflow Task 4 주기 갱신 |
| FR-14 | Athena 특화 지식 (Presto SQL 규칙) | `seed_chromadb.py` 1회 적재 | Admin API + Airflow Task 4 주기 갱신 |
| FR-15 | 정책 데이터 (코드값 매핑, 계산식) | `seed_chromadb.py` 1회 적재 | Admin API + Airflow Task 4 주기 갱신 |

### 4.2 신규 Admin API 설계

```
POST /admin/training-data/{type}
  - type: term | rule | policy
  - Auth: Admin API Key (X-Admin-Key 헤더)
  - Body:
    {
      "content": "문서 내용",
      "tags": ["CTR", "광고"]
    }
  - 동작: vanna.train(documentation=content) 즉시 실행 → ChromaDB 추가
  - 응답: { "training_id": "uuid", "status": "added" }

DELETE /admin/training-data/{training_id}
  - 기존 미해결 이슈로 추적 중인 DELETE 엔드포인트 구현
```

### 4.3 ChromaDB Collection 구조

Phase 2에서 Vanna 기본 Collection 구조(`sql-ddl`, `sql-documentation`, `sql-qa`)는 변경하지 않는다. 별도 DynamoDB 테이블(`capa-{env}-training-metadata`)로 학습 데이터의 출처·일시·타입을 추적한다.

```python
# DynamoDB training-metadata 스키마
{
    "training_id": "uuid",        # PK
    "data_type": "term|rule|policy|feedback",
    "source": "manual_seed|admin_api|airflow_sync",
    "content_hash": "sha256",     # 중복 방지
    "created_at": "ISO8601",
    "tags": ["CTR", "광고"],
    "ttl": unix_epoch              # 자동 삭제 (선택)
}
```

> **Vanna SDK 제약**: Vanna SDK가 ChromaDB metadata 커스텀 필드를 직접 지원하지 않으므로, 별도 메타데이터 인덱스(DynamoDB)를 병행 관리한다.

---

## 5. FR-16~17: 피드백 루프 + 중복 쿼리 방지 설계

### 5.1 FR-16: 피드백 루프 품질 제어
...
#### 5.1.3 FeedbackManager 수정 (Phase 2)
...
> **Phase 1 대비 변경점**: `RedashClient.create_query()` → `create_or_reuse_query()` 로 변경. SQL 해시 인덱스(DynamoDB) 기반 기존 Redash 쿼리 재사용.

#### 5.2.1 SQL 해시 계산

```python
# src/pipeline/sql_hash.py (신규)
import re
import hashlib
...
#### 5.2.2 Redash 쿼리 재사용 로직

```
[RedashClient.create_or_reuse_query()]
  1. compute_sql_hash(generated_sql) → sql_hash
  2. DynamoDB `capa-{env}-query-hash-index` 조회:
     - PK: sql_hash
     - 있으면 → 기존 redash_query_id 반환 (Redash 신규 생성 스킵)
     - 없으면 → 3단계로
  3. Redash POST /api/queries → 신규 query_id 생성
  4. DynamoDB `query-hash-index` 에 저장:
     {
       "sql_hash": hash,       # PK
       "query_id": id,
       "created_at": "ISO8601",
...
```
extract_pending_feedbacks
        │
        ▼
validate_and_deduplicate
        │
        ▼
batch_train_chromadb
        │
        ▼
sync_knowledge_base
        │
...
#### Task 4: `sync_knowledge_base`

```python
def sync_knowledge_base(**kwargs) -> dict:
    """S3 knowledge-base에서 신규/변경된 비즈니스 용어·정책 문서를 ChromaDB에 동기화"""
    # 1. S3 s3://capa-{env}-knowledge-base/ 에서 최근 변경 파일 목록 조회
    # 2. 각 파일의 content_hash와 DynamoDB training-metadata 비교
    # 3. 신규/변경된 문서만 vanna.train(documentation=) 실행
    # 4. DynamoDB training-metadata 업데이트
```

- **입력**: S3 `capa-{env}-knowledge-base/` 버킷
- **출력**: 동기화 결과 통계
- **구조**: `terms/`, `rules/`, `policies/` 디렉토리별 분류

#### Task 5: `report_results`

```python
def report_results(**kwargs) -> None:
    """실행 결과를 Slack #capa-alerts 채널로 요약 전송"""
    # Task 3, 4의 결과를 XCom에서 취합
...
GET /query/{task_id} → 폴링
  ...
  - status: "expired"    → 410 Gone (TTL 만료)
```

### 7.2 AsyncQueryManager
...
#### 8.1.2 `capa-{env}-pending-feedbacks`
...
#### 8.1.3 `capa-{env}-query-hash-index`

| 속성 | 타입 | 역할 |
|------|------|------|
| `sql_hash` (PK) | String | SHA-256 해시 |
| `redash_query_id` | Number | Redash query ID |
| `created_at` | String | ISO 8601 |
| `ttl` | Number | Unix epoch + 90일 |

#### 8.1.4 `capa-{env}-training-metadata`

| 속성 | 타입 | 역할 |
|------|------|------|
| `training_id` (PK) | String | UUID |
| `data_type` | String | term / rule / policy / feedback |
| `source` | String | manual_seed / admin_api / airflow_sync |
| `content_hash` | String | SHA-256 (중복 방지) |
| `created_at` | String | ISO 8601 |
| `tags` | List | 태그 목록 |
| `ttl` | Number | Unix epoch (선택) |

> **용도**: Vanna SDK가 ChromaDB 메타데이터 커스텀 필드를 지원하지 않으므로, 학습 데이터의 출처/일시/타입 추적을 위한 별도 인덱스 테이블.

#### 8.1.5 `capa-{env}-async-tasks`
...
| `created_at` | String | ISO 8601 |
| `completed_at` | String | |
| `result` | Map | QueryResponse 직렬화 |
| `error` | Map | ErrorResponse 직렬화 |
| `ttl` | Number | Unix epoch + 24시간 |

### 8.2 HistoryRecorder 수정
...
| 변수명 | 분류 | 예시값 | 설명 |
|--------|------|--------|------|
| `PHASE2_RAG_ENABLED` | ConfigMap | `false` | 3단계 RAG 활성화 (FR-12) |
| `ASYNC_QUERY_ENABLED` | ConfigMap | `false` | 비동기 쿼리 모드 활성화 |
| `DYNAMODB_ENABLED` | ConfigMap | `false` | DynamoDB 저장소 전환 플래그 |
...
| `DYNAMODB_FEEDBACK_TABLE` | ConfigMap | `capa-dev-pending-feedbacks` | Pending Feedback DynamoDB 테이블명 |
| `DYNAMODB_HASH_INDEX_TABLE` | ConfigMap | `capa-dev-query-hash-index` | SQL 해시 인덱스 DynamoDB 테이블명 |
| `DYNAMODB_ASYNC_TASK_TABLE` | ConfigMap | `capa-dev-async-tasks` | 비동기 Task DynamoDB 테이블명 |
| `DYNAMODB_TRAINING_META_TABLE` | ConfigMap | `capa-dev-training-metadata` | 학습 데이터 메타데이터 테이블명 |
...
| `KNOWLEDGE_BASE_S3_BUCKET` | ConfigMap | `capa-dev-knowledge-base` | 지식 베이스 S3 버킷 |
| `ADMIN_API_KEY` | **Secret** | `(secret)` | Admin API 인증 키 |
...
## 10. K8s / Terraform 변경사항

### 10.1 DynamoDB 리소스 (Terraform)
...
resource "aws_dynamodb_table" "query_hash_index" {
  name         = "capa-${var.env}-query-hash-index"
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "sql_hash"
...
resource "aws_dynamodb_table" "training_metadata" {
  name         = "capa-${var.env}-training-metadata"
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "training_id"
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          aws_dynamodb_table.query_hash_index.arn,
          aws_dynamodb_table.training_metadata.arn,
...
| 항목 | 변경 내용 |
|------|----------|
| vanna-api ConfigMap | DynamoDB 테이블명 5개, Reranker 설정, 전환 플래그 3개 추가 |
| vanna-api Secret | `ADMIN_API_KEY` 추가 |
| vanna-api PVC | Reranker 모델 캐시용 PersistentVolumeClaim (1Gi) |
| vanna-api 메모리 | 1.5Gi → **2Gi** (Reranker 모델 로드 + DynamoDB 클라이언트) |
...
resource "aws_s3_bucket" "knowledge_base" {
  bucket = "capa-${var.env}-knowledge-base"
...
| `src/pipeline/sql_hash.py` | SQL 정규화 + SHA-256 해시 | FR-17 |
...
| `src/stores/dynamodb_hash_index.py` | DynamoDB SQL Hash Index Store | FR-17 |
| `services/airflow-dags/capa_chromadb_refresh.py` | Airflow DAG | FR-18 |
| `infrastructure/terraform/13-dynamodb.tf` | DynamoDB 테이블 정의 | 인프라 |
...
| `src/redash_client.py` | `create_or_reuse_query()` 추가 | FR-17 |
| `src/main.py` | BackgroundTasks 비동기 + Admin API + GET /query/{task_id} | 전체 |
| `src/query_pipeline.py` | AsyncQueryManager 연동 | BackgroundTasks |
| `src/models/domain.py` | PipelineContext에 sql_hash 필드 추가 | FR-17 |
| `services/slack-bot/app.py` | 비동기 폴링 방식으로 전환 | BackgroundTasks |
| `infrastructure/terraform/11-k8s-apps.tf` | ConfigMap/Secret/PVC/메모리 변경 | 인프라 |
| `requirements.txt` | `sentence-transformers`, `boto3` DynamoDB 추가 | 의존성 |

---

## 11.3 NFR-05: SQL 생성 프롬프트 구조 설계 (영어 XML 기반)
...
| D-03 | SQL 해시 인덱스를 DynamoDB에 저장 | Redash API 검색 (`GET /api/queries?search=`) | Redash 검색은 이름 기반으로 불확실성 높음, DynamoDB 해시 PK 조회가 O(1) 확정적 |
| D-04 | BackgroundTasks 비동기 응답 채택 | WebSocket, SSE (Server-Sent Events) | FastAPI 내장 기능으로 추가 인프라 불필요, Slack Bot은 이미 폴링 패턴 사용 중 |
| D-05 | Airflow DAG 배치 학습 (즉시 학습 폐기) | 즉시 학습 유지 + 품질 필터 추가 | Plan FR-16 명시적 요구, 검증 없는 즉시 학습은 ChromaDB 품질 저하 위험 |
| D-06 | ChromaDB Collection 구조 변경 없음 | 커스텀 Collection 추가 | Vanna SDK 호환성 유지, 메타데이터 관리는 DynamoDB 별도 테이블로 분리 |
| D-07 | `ASYNC_QUERY_ENABLED` 플래그로 점진적 전환 | 즉시 비동기 전환 | Phase 1 → Phase 2 전환 기간 안정성 확보 |
| D-08 | 지식 베이스 소스를 S3 버킷으로 관리 | Git 리포지토리 직접 참조 | Airflow에서 S3 접근이 가장 자연스러움, 비개발자도 파일 업로드 가능 |

---

## 13. 에이전트 기여 내역 (Agent Attribution)
...
| `enterprise-expert` | claude-sonnet-4-6 | 3단계 RAG 설계(FR-12), ChromaDB 지속 관리(FR-13~15), 피드백 루프(FR-16), 중복 쿼리 방지(FR-17), BackgroundTasks 아키텍처, History 전환 |
...
| §4 FR-13~15 ChromaDB 관리 | `enterprise-expert` | Admin API 설계, 메타데이터 관리 전략 |
| §5 FR-16~17 피드백/중복 | `enterprise-expert` | FeedbackManager 수정, SQL 해시 설계 |
| §6 FR-18 Airflow DAG | `infra-architect` | DAG 정의, Task 그래프, 각 Task 상세 로직 |
| §7 BackgroundTasks | `enterprise-expert` + `infra-architect` | 아키텍처는 enterprise, 인프라/상태관리는 infra |
| §8 DynamoDB 설계 | `infra-architect` | 테이블 스키마, GSI, TTL 설계 |
| §9 환경 변수 | `infra-architect` | 신규 변수 목록 전체 |
| §10 Terraform | `infra-architect` | DynamoDB 5테이블 리소스, IAM 정책, S3 버킷 |
| §11 파일 목록 | `cto-lead` | 전체 신규/수정 파일 통합 정리 |
| §12 Decision Log | `cto-lead` | 전체 결정 근거 종합 |
