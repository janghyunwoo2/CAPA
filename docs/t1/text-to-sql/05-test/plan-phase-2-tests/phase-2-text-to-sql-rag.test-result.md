...
| TC-P2-U14 | 피드백 루프 — DynamoDB pending 저장 (FR-17) | test_phase2_dynamodb_feedback.py | save_pending 정상 저장 | `history_id="h1"`, `sql="SELECT 1"` | UUID 반환, status=pending | `assert len(feedback_id) == 36` | ✅ PASS | |
| TC-P2-U15 | 피드백 루프 — DynamoDB pending 저장 (FR-17) | test_phase2_dynamodb_feedback.py | save_pending sql_hash 자동 계산 | `sql="SELECT 1"` | `item["sql_hash"] == expected_hash` | `assert item["sql_hash"] == expected_hash` | ✅ PASS | |
| TC-P2-U16 | 피드백 루프 — DynamoDB pending 저장 (FR-17) | test_phase2_dynamodb_feedback.py | save_pending TTL 90일 | 현재 시간 기준 | TTL ≈ now+90일 | `assert abs(actual_ttl - expected) < 5` | ✅ PASS | |
| TC-P2-U17 | 피드백 루프 — DynamoDB pending 저장 (FR-17) | test_phase2_dynamodb_feedback.py | save_pending DynamoDB 장애 | `put_item` → ClientError | 예외 미전파, UUID 반환 | `assert feedback_id is not None` | ✅ PASS | |
| TC-P2-U18 | 피드백 루프 — DynamoDB pending 저장 (FR-17) | test_phase2_dynamodb_feedback.py | update_status 정상 업데이트 | `status="trained"` | `item["status"] == "trained"` | `assert item["status"] == "trained"` | ✅ PASS | |
| TC-P2-U19 | 피드백 루프 — DynamoDB pending 저장 (FR-17) | test_phase2_dynamodb_feedback.py | update_status DynamoDB 장애 | `update_item` → ClientError | False 반환 | `assert result is False` | ✅ PASS | |
| TC-P2-U20 | Redash 쿼리 ID 캐시 (FR-16) | test_phase2_redash_cache.py | create_or_reuse_query 캐시 히트 | DynamoDB에 sql_hash 존재 | `42` 반환, POST 미호출 | `assert result == 42` | ✅ PASS | |
| TC-P2-U21 | Redash 쿼리 ID 캐시 (FR-16) | test_phase2_redash_cache.py | create_or_reuse_query 캐시 미스 | sql_hash 없음, POST → 99 | `99` 반환, DynamoDB 저장 | `assert result == 99` | ✅ PASS | |
| TC-P2-U22 | Redash 쿼리 ID 캐시 (FR-16) | test_phase2_redash_cache.py | create_or_reuse_query DynamoDB 장애 | `get_item` → ClientError | `77` 반환, 예외 미전파 | `assert result == 77` | ✅ PASS | |
| TC-P2-U23 | Redash 쿼리 ID 캐시 (FR-16) | test_phase2_redash_cache.py | create_or_reuse_query table=None | `dynamodb_table=None` | `55` 반환 | `assert result == 55` | ✅ PASS | |
| TC-P2-U24 | Redash 쿼리 ID 캐시 (FR-16) | test_phase2_redash_cache.py | create_or_reuse_query TTL 저장 | 캐시 미스 시 DynamoDB 저장 | TTL ≈ now+90일 | `assert abs(actual_ttl - expected) < 5` | ✅ PASS | |
...
| TC-P2-U39 | SQL 해시 파이프라인 연동 | test_phase2_pipeline_sql_hash.py | sql_hash Redash 경로 할당 | SQL → ctx | `ctx.sql_hash == expected_hash` | `assert len(ctx.sql_hash) == 64` | ✅ PASS | |
| TC-P2-U40 | SQL 해시 파이프라인 연동 | test_phase2_pipeline_sql_hash.py | sql_hash Athena fallback = None | Athena 경로 | `ctx.sql_hash is None` | `assert ctx.sql_hash is None` | ✅ PASS | |
...
| TC-P2-U53 | 피드백 루프 — FeedbackManager (FR-17) | test_phase2_feedback_manager.py | record_positive Phase2 pending 저장 | `history_id="h1"` | (False, msg), pending 1건, train 미호출 | `assert result[0] is False` | ✅ PASS | |
| TC-P2-U54 | 피드백 루프 — FeedbackManager (FR-17) | test_phase2_feedback_manager.py | record_positive history 없음 | `history_id="nonexistent"` | (False, 이력 관련 메시지) | `assert "이력" in result[1]` | ✅ PASS | |
| TC-P2-U55 | 피드백 루프 — FeedbackManager (FR-17) | test_phase2_feedback_manager.py | record_positive DynamoDB 장애 | `save_pending` → Exception | 예외 미전파 | `assert result[0] is False` | ✅ PASS | |
