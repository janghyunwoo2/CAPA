...
> **작성일**: 2026-03-15 (v2 — 시나리오 기반 전면 보강)
> **담당**: t1
...
배포 전 (로컬)                    배포 후 (EKS)
─────────────────────          ──────────────────────────────────
Phase 1: 단위 테스트 (pytest)   Phase 3: E2E 시나리오 A, B
Phase 2: 통합 테스트            Phase 3: 예외 시나리오 EX-1 ~ EX-10
                                Phase 4: SQL 품질 평가 (LLM-as-Judge)
```
...
```
[Phase 1] pytest 단위 테스트 → 전체 통과 필수
              도구: pytest (bkit 없음)
    ↓
[Phase 2] 로컬 통합 테스트 (ChromaDB + 파이프라인)
...
> **성공 기준**: 전체 통과 + 커버리지 **80% 이상** (plan §11.6 기준)
