...
> **관련 문서**: `../00-기타/사전조사/data_schema_design.md` (3+2 테이블 설계), `../01-plan/features/text-to-sql.plan.md` (Plan)
...
기존 `../00-기타/사전조사/data_schema_design.md`의 3+2 테이블 구조를 Pydantic v2 모델로 표현한다. 이 모델들은 ChromaDB 학습 데이터 등록 시 DDL 자동 생성 및 검증에 활용된다.
...
# ⚠️ 실제 Athena 테이블 기준 (docs/t1/text-to-sql/00_mvp_develop/02-design/05-sample-queries.md)
...
`vanna.train(question=, sql=)` 형태로 질문-SQL 쌍을 학습한다. Plan 문서의 10개 초기 구축 대상 + `../00-기타/사전조사/target_user_queries.md`의 핵심 질문을 포함한다.
...
| 에이전트 | 모델 | 수행 작업 |
|---------|------|----------|
| `data-modeler` | Opus 4.6 | 기존 ../00-기타/사전조사/data_schema_design.md 분석, Plan 문서의 FR 요구사항 매핑, Pydantic v2 도메인 모델 설계, ChromaDB 3개 컬렉션 학습 데이터 구조 설계, 피드백 루프 스키마 설계, Redash 연동 데이터 모델 설계 |

### 문서 섹션별 주요 기여

| 섹션 | 기여 에이전트 | 기여 내용 |
|------|-------------|----------|
| §4.1 Pydantic 도메인 모델 | `data-modeler` | ../00-기타/사전조사/data_schema_design.md 3+2 테이블 → Pydantic v2 모델 변환, MVP QueryResponse 확장 설계 |
