...
| **참고 문서** | `docs/t1/text-to-sql/00_mvp_develop/01-plan/features/text-to-sql.plan.md` §7 Phase 2, §12.4 |
| **선행 문서** | `docs/t1/text-to-sql/00_mvp_develop/02-design/features/phase-1-text-to-sql.design.md` |
...
[Reranker 재평가 — Cross-Encoder]
  - 모델: jinaai/jina-reranker-v2-base-multilingual (sentence-transformers, trust_remote_code=True)
  - 입력: (질문, 후보 문서) 쌍
...
| 항목 | 선택 | 근거 |
|------|------|------|
| 모델 | `jinaai/jina-reranker-v2-base-multilingual` | SQL-aware 설계, TableSearch recall@3: 93.31%, HuggingFace Text-to-SQL 공식 예제 채택, 278M |
| 라이브러리 | `sentence-transformers>=2.6.1` | HuggingFace 공식, Reranker API 안정적 (`trust_remote_code=True` 필요) |
| 대안 | `cross-encoder/ms-marco-MiniLM-L-6-v2` 제외 | MS MARCO(웹 검색) 훈련 모델 — SQL DDL 스키마를 저평가, 평가 0% 원인으로 확인 (2026-03-23) |
| 대안 | Cohere Rerank API 제외 | 외부 API 의존성 + 비용 발생 → 자체 모델 선호 |
| 컨테이너 | Dockerfile 빌드 시 사전 다운로드 | `RUN python -c "CrossEncoder('jinaai/...', trust_remote_code=True)"` 로 런타임 다운로드 방지 |
...
### 3.7 Step 4 → Step 5 RAG 컨텍스트 전달

> **수정 이력 (2026-03-23)**: `rag_context` 미사용 버그 수정 — `SQLGenerator.generate()`가 `rag_context` 인자를 받으면서도 프롬프트에 주입하지 않아 Step 4 결과가 버려지던 문제 해결. 동시에 Vanna 내부 RAG 중복 호출(ChromaDB 2회 검색) 제거.

#### 수정 전 (버그)

```
Step 4: RAGRetriever → rag_context 생성
              ↓ (버려짐 ❌)
Step 5: vanna.generate_sql(question=prompt)
        └→ Vanna 내부에서 ChromaDB 재검색 (중복)
        └→ Claude 호출
```
...
Step 4: RAGRetriever → rag_context 생성
              ↓
Step 5: SQLGenerator
        ├─ rag_context 있음 → rag_block 조립 + vanna.submit_prompt([...])
        │                      (Vanna 내부 RAG 우회, Claude 직접 호출)
        └─ rag_context 없음 → vanna.generate_sql() (Phase 1 하위 호환)
```
...
`history_block`(`MULTI_TURN_ENABLED`)과 `rag_block`(`PHASE2_RAG_ENABLED`)은 독립적으로 조립되어 동일한 프롬프트에 포함됨. 두 플래그 동시 활성화 시에도 정상 동작.
...
> **⛔ 설계 변경 (2026-03-21)**: DynamoDB `pending_feedbacks` 저장 및 Airflow 배치 학습 중단. 피드백 기록(positive/negative 표기)은 `query-history` 테이블에 유지. `feedback_manager.py`의 `save_pending()` 호출 블록은 주석처리 상태로 보존.

> **Hotfix FR-16a (2026-03-21)**: `sql_generator.py` 날짜 컨텍스트 수정 — ChromaDB Few-shot 예시의 하드코딩 파티션 날짜(`year='2026' AND month='03' AND day='13'` 형식)를 LLM이 복사하는 환각 이슈 발견. 파티션 형식(`year/month/day` STRING 2자리) 명시 및 예시 값 복사 금지 경고 문구 추가.
...
> **⛔ 중단됨 (2026-03-21)**: `capa_chromadb_refresh` DAG 및 관련 Task 함수가 주석처리되었습니다. 코드는 보존되어 있으며 재활성화 시 주석 해제로 복구 가능합니다.

> **Phase 1 대비 변경점**: 완전 신규. `capa_chromadb_refresh` DAG가 매주 월요일 09:00 KST에 실행되어 피드백 검증 + ChromaDB 학습을 자동화. (중단됨)
...
| `RERANKER_MODEL_NAME` | ConfigMap | `jinaai/jina-reranker-v2-base-multilingual` | Reranker 모델명 |
...
| # | 결정 | 대안 | 채택 근거 |
|---|------|------|----------|
| D-01 | ~~Reranker로 `cross-encoder/ms-marco-MiniLM-L-6-v2` 선택~~ → **`jinaai/jina-reranker-v2-base-multilingual`로 변경 (2026-03-23)** | Cohere Rerank API | ms-marco는 웹 검색 모델로 SQL DDL 저평가 → Spider 평가 0% 원인. Jina v2는 SQL-aware, TableSearch recall@3: 93.31%, HuggingFace Text-to-SQL 공식 채택 |
