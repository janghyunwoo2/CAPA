...
> **Analyst**: t1
> **Date**: 2026-03-19
> **Design Doc**: [../../02-design/features/phase-2-text-to-sql-rag.design.md](../../02-design/features/phase-2-text-to-sql-rag.design.md)
> **Status**: Approved
...
### 1.3 설계 변경 이력 (Gap 분석 이후)

| 날짜 | 항목 | 변경 내용 | 영향 |
|------|------|----------|------|
| 2026-03-21 | **FR-16a Hotfix** (날짜 환각 방지) | `sql_generator.py` 날짜 컨텍스트에 파티션 형식(`year/month/day` STRING 2자리) 및 예시 값 복사 금지 경고 추가 | sql_generator.py 수정, TC-P2-U58 신규 추가 |
| 2026-03-21 | **FR-18 중단** (Airflow DAG 중단) | `capa_chromadb_refresh.py` Task 함수·DAG 정의 주석처리. `feedback_manager.py`의 `save_pending()` 호출 주석처리. 테스트(`test_phase2_chromadb_refresh.py`) 전체 비활성화 | TC-P2-U30~U35, U56 비활성. TC-IT-P2-07, E2E Step 5 보류 |
...
| 항목 | 파일:행 | 상태 |
|------|--------|:----:|
| 설계서 §9.1 DynamoDB | `../../02-design/features/phase-2-text-to-sql-rag.design.md:930-932` | ✅ |
