...
| **작성일**    | 2026-03-11                                                                      |
| **담당**      | t1                                                                              |
| **참고 문서** | `docs/t1/text-to-sql/00_mvp_develop/00-기타/사전조사/reference_summary.md` (DableTalk, 물어보새, InsightLens) |
...
| FR-16 | **피드백 루프 품질 제어**: Phase 1(FR-21)의 즉시 학습 방식을 폐기하고 DynamoDB query-history에 피드백 기록. ~~DynamoDB pending_feedbacks 저장 및 Airflow 배치 학습~~ — **⛔ Airflow 연동 중단 (2026-03-21).** 피드백 기록(positive/negative 표기)은 유지. Hotfix: `sql_generator.py` 날짜 컨텍스트에 파티션 형식 및 예시 값 복사 금지 경고 추가 (FR-16a, 2026-03-21) | InsightLens + DableTalk |
| FR-18 | **Airflow DAG 연동**: ~~매주 월요일 09:00 실행하여 DynamoDB `pending_feedbacks`의 피드백 검증(EXPLAIN + 해시 중복 제거) 및 ChromaDB 배치 학습 수행 (3 Task: extract → validate → batch_train)~~ — **⛔ 중단됨 (2026-03-21).** DAG 코드는 주석 처리 상태로 보존. 재활성화 조건 미정. | 물어보새 |
...
| FR-23 | **Redash 차트 자동 생성**: query_id 발급 후 Redash Visualizations API(`POST /api/visualizations`)로 bar/line 차트를 자동 생성. Slack에 전달되는 `redash_url`이 차트 포함 대시보드 링크로 제공되어 사용자가 링크 클릭 시 Redash에서 인터랙티브 차트를 확인할 수 있음. Phase 1의 matplotlib PNG(Slack 직접 첨부)는 유지하며 Redash 차트를 추가 제공 | UX 개선 |
...
| 문서                                                         | 적용 패턴                                                    |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| `docs/t1/text-to-sql/00_mvp_develop/00-기타/사전조사/reference_summary.md` §1 DableTalk   | 의도 분류, SQL EXPLAIN, Redash 연동, History, 실패 투명성    |
| `docs/t1/text-to-sql/00_mvp_develop/00-기타/사전조사/reference_summary.md` §2 물어보새    | 비즈니스 용어 사전, Few-shot SQL, Router 의도 분류           |
| `docs/t1/text-to-sql/00_mvp_develop/00-기타/사전조사/reference_summary.md` §3 InsightLens | 질문 정제, 키워드 추출, 3단계 RAG, XML 프롬프트, 피드백 루프 |
...
> **상세 시나리오 문서**: `docs/t1/text-to-sql/00-보고서../../00-기타/e2e-scenario.md` 참고
...
> **상세 내용**: `docs/t1/text-to-sql/00_mvp_develop/05-test/plan-phase-2-tests/phase-2-text-to-sql-rag.test-plan.md` 참고
...
| **FR-16** | 피드백 루프 자동화 | `stores/dynamodb_feedback.py`, `feedback_manager.py` | TC-P2-U14~U19, U53~U55 (9건) + **TC-P2-U58** (FR-16a 날짜 환각 방지) | TC-P2-04, TC-P2-14 | TC-P2-E2E-01 (단계 6) | DynamoDB query-history positive/negative 기록만 수행 (pending 저장 중단) |
| **FR-18** | ~~Airflow DAG 배치 학습~~ **⛔ 중단됨** | `airflow-dags/dags/capa_chromadb_refresh.py` (주석처리) | ~~TC-P2-U30~U35, U56 (6건) — **비활성**~~ | ~~TC-P2-08, TC-P2-09, TC-P2-15~~ | ~~TC-P2-E2E-01 (단계 7~9)~~ | 2026-03-21 중단. 테스트 파일 주석처리. |
