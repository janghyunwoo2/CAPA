...
graph TD
    ...
    subgraph "데이터 수집 (Ingestion)"
        B["AWS Kinesis Data Stream<br/>(실시간 스트림)"]
        C["AWS Kinesis Data Firehose<br/>(Parquet 변환 + 동적 파티셔닝)"]
    end
    ...
    %% 연결 관계 정의
    A -- "광고 로그 전송<br/>(Kinesis Producer)" --> B
    B -- "스트림 소비" --> C
    C -- "Parquet 변환 + 파티셔닝<br/>(event_type/year/month/day)" --> D
    F -- "배치 스케줄링" --> G
