...
Airflow는 이제 Terraform으로 관리됩니다. 아래 명령어로 배포를 진행합니다.
...
# Airflow 모듈만 타겟팅하여 배포 (권장: 전체 통합 배포)
terraform apply -target=helm_release.airflow
```

**참고**: `helm_release` 리소스가 `airflow` 네임스페이스를 자동으로 생성하도록 설정되어 있습니다 (`create_namespace = true`).
...
# airflow  airflow     1          ...       deployed   airflow-1.12.0  2.8.1
```
