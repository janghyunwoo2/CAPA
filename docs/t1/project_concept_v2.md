...
> AI가 이상을 먼저 알려주며, 리포트를 자동으로 생성하는 **대화형 분석 플랫폼**

### 핵심 컨셉

> **데이터 분석을 일회성 요청(Ticket)이 아니라,  
> 결과를 보며 점진적으로 보완해 나가는 대화형 과정으로 전환한다.**

```
[기존 분석 방식]
요청자 → 티켓 → 데이터팀 → SQL → 결과 → 요청자 → 추가 요청 → 반복...
       ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                         3시간 ~ 3일
...
요청자 ↔ CAPA AI ↔ 데이터
       ~~~~~~~~~~~
          1분
"어제 CTR top 5" → 결과 → "광고주별로" → 결과 → "차트로" → 결과
```
...
## 해결 방향: AI 기반 데이터 분석 헬퍼

CAPA는 기존 SQL 기반 데이터 분석 환경 위에 **AI 기반 데이터 분석 헬퍼**를 결합합니다.
...
한 번의 질문으로 끝나지 않습니다. 연속적인 대화로 분석을 심화할 수 있습니다:

| 후속 요청 | AI 응답 |
|----------|---------|
| "광고주별로 분리해줘" | GROUP BY advertiser_id 추가 |
| "이상치 제거하고 다시" | WHERE 조건에 outlier 필터 추가 |
| "전일 대비 증감 보여줘" | 전일 데이터 JOIN + 증감률 계산 |
| "이걸 차트로 보여줘" | 시각화 제공 |
...
│   [분석 경험 개선]                    [신뢰성 보장]                  │
│                                                                     │
│   🗣️ Ask   - 자연어 → SQL            🔔 Alert  - 이상 자동 감지     │
│   💬 Chat  - 대화형 분석              📊 Dashboard - KPI 시각화     │
│   📝 Report - 리포트 자동화                                         │
│                                                                     │
│   "결과를 보며 점진적으로             "문제가 생기면 AI가 먼저       │
│    보완하는 대화형 분석"               알려주는 선제 대응"           │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| 기능 | 설명 | 대화형 특성 | 해결하는 문제 |
|------|------|------------|--------------|
| 🗣️ **Ask** | 자연어로 질문 → AI가 SQL로 답변 | 후속 질문으로 분석 심화 | 분석 도구 진입장벽 |
| 💬 **Chat** | 슬랙에서 바로 분석 | 스레드로 맥락 유지 | 데이터 접근성 |
| 📝 **Report** | 주간/월간 리포트 자동 생성 | "다른 기준으로" 재생성 | 리포트 생산성 |
...
Ask의 핵심 가치는 **한 번의 질문으로 끝나지 않는다**는 점입니다.

```
[대화형 분석 시나리오]

사용자: "어제 캠페인별 CTR top 5 알려줘"
CAPA:   [결과 테이블] + [생성된 SQL]

사용자: "광고주별로 분리해서 보여줘"
CAPA:   [광고주별 결과] + [수정된 SQL - GROUP BY advertiser_id 추가]

사용자: "CTR 1% 미만인 건 제외하고"
CAPA:   [필터된 결과] + [수정된 SQL - HAVING ctr >= 0.01 추가]

사용자: "전일 대비 증감도 보여줘"
CAPA:   [증감 포함 결과] + [수정된 SQL - 전일 데이터 JOIN]
...
| 구분 | 기존 (티켓 방식) | CAPA (대화형) |
|------|-----------------|--------------|
| 추가 요청 | 새 티켓 → 대기 | 바로 후속 질문 |
| 컨텍스트 | 매번 다시 설명 | 이전 대화 맥락 유지 |
| 반복 비용 | 요청마다 수 시간 | 후속 질문당 수 초 |
| 분석 깊이 | 1회성으로 끝남 | 점진적 심화 가능 |
...
MVP에서는 Lambda + 통계적 방식으로 시작하고, 다음 단계에서 ML 기반으로 고도화합니다:

| 단계 | 방식 | 기술 | 탐지 범위 |
|:----:|------|------|----------|
| MVP | 통계적 (현재) | Lambda + 이동평균 | 로그 유입량 |
| 확장 1 | 시계열 예측 | **Prophet** / ARIMA | 요일/시간대 패턴 반영 |
| 확장 2 | 이상치 탐지 | SageMaker + Isolation Forest | 다변량 이상 패턴 |

> **왜 Prophet 추천?**  
> 광고 데이터는 요일별/시간대별 계절성이 뚜렷합니다. Prophet은 이런 계절성을 자동으로 학습하여 "월요일 오전이라 트래픽이 적은 건지" vs "진짜 이상인지" 구분할 수 있습니다.

자세한 ML 이상 탐지 구현은 **AI Layer 확장 아이디어 → 6. ML 기반 이상 탐지** 섹션 참고
...
Chat의 핵심 가치는 **슬랙 스레드로 맥락을 유지**하며 분석을 심화할 수 있다는 점입니다.

```
#marketing 채널
├─ 🧑 PM: @capa-bot 어제 전환율 top 5 캠페인
│   └─ 🤖 CAPA: [결과 테이블]
│       └─ 🧑 PM: 광고주별로 분리해줘
│           └─ 🤖 CAPA: [광고주별 결과]
│               └─ 🧑 PM: A광고주만 자세히
...
| 구분 | 분석 도구 (Redash 등) | CAPA Chat |
|------|---------------------|-----------|
| 접근 | 별도 웹사이트 접속 | 업무 중인 슬랙에서 바로 |
| 공유 | 링크 복사 → 붙여넣기 | 채널에서 실시간 공유 |
| 히스토리 | 개인 대시보드에 저장 | 팀 채널에 자동 기록 |
| 협업 | 각자 따로 분석 | 스레드에서 함께 토론 |
...
- 이상 감지는 Alert 기능(Lambda + 통계적 탐지)으로 별도 처리
...
"병목은 분석 자체가 아니라, 요청-결과 주고받는 과정"
                    ↓
"CAPA는 대화형 분석으로 이 병목을 제거한다"
                    ↓
      [MVP: Ask, Alert, Report, Chat, Dashboard]
                    ↓
      [확장: dbt, Prophet/ML, LLM-as-Judge, ...]
```
