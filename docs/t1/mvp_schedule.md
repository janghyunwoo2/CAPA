...
### Phase 5: Batch + Dashboard (Day 8-9)

**목표**: Airflow로 집계 쿼리 실행 → Redash에 기본 KPI 대시보드 구성

```
Airflow DAG → Athena 집계 쿼리 → Redash 대시보드
```

**체크리스트:**
- [ ] Airflow 환경 구성 (로컬 또는 EKS)
- [ ] 일별 집계 DAG 작성 (노출/클릭/전환)
- [ ] Redash 설치 (Docker 또는 EC2)
- [ ] Athena Data Source 연결
- [ ] 기본 KPI 대시보드 구성 (차트 3개)

**완료 기준**: Redash에서 일별 노출/클릭/전환 차트가 보이면 성공
